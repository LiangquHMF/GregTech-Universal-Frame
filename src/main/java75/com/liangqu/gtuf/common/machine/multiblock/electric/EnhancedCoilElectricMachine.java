 ...
 * </p>
 * <p>
 * <b>版本分源（GTM ≥ 7.5）</b>：本文件位于 {@code src/main/java75}，仅在 GTM 7.5.x 构建时参与编译。
 * 温度/电压门控失败时用 {@link ModifierFunction#cancel(Component)} 携带明确失败原因
 * （镜像 {@link GTRecipeModifiers#ebfOverclock}），控制器 GUI 会显示
 * 「Coil Temperature Too Low / Voltage Tier Too Low」而非笼统的「Recipe Modifier Fail」。
 * 7.1.4~7.4.1 无该 API，见 {@code src/main/java-legacy-pipe} 下的同名类（返 {@link ModifierFunction#NULL}）。
 * </p>
 */
public class EnhancedCoilElectricMachine extends CoilWorkableElectricMultiblockMachine {
     ...
     * {@link ModifierFunction#cancel(Component)}（配方不启动，且携带具体失败原因，
     * 经 {@link com.gregtechceu.gtceu.api.recipe.modifier.RecipeModifierList} 存入机器
     * failureReasonMap，控制器 GUI / Jade 直接显示原因文本，而非笼统的 Recipe Modifier Fail）。
     * 不带 {@code ebf_temp} 的配方跳过此检查。
     * </p>
     */
