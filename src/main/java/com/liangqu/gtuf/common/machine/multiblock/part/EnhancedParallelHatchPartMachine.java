 ...
 *
 * <p>与原生最大的区别是并行上限公式：原生 {@code (int) Math.pow(4, tier - EV)} 在 EV 以下为负指数 →
 * 并行数恒为 0，导致 LV/MV/HV 等级完全不可用。本机改为每升一级并行数 ×2：</p>
 * <pre>
...
public class EnhancedParallelHatchPartMachine extends TieredPartMachine implements IFancyUIMachine, IParallelHatch {
     ...
     * <p>前期（LV~IV）每级 ×2：1, 2, 4, 8, 16；后期（LuV 起）每级 ×4：64, 256, 1024。
     * 相比原生公式 4^(tier-4)（EV 以下为 0、UV=256），本机保证低等级可用，且后期增幅不低于原生
     * （UV=1024 为原生的 4 倍）。如需修改公式，直接覆盖此方法。</p>
     */
