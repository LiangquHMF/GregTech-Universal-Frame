 ...
 * 不打包进公开发布 jar。
 *
 * <p>通过 {@code @Mod.EventBusSubscriber} 让 Forge 在 mod 加载时自动发现本类并订阅机器注册事件，
 * 在框架注册类 {@code GTUF_Machines} 之后注册测试机器。main 源码集完全不引用本类，
 * 保证公开发布剔除 testmod 后无断链。</p>
 *
 * <p>同时通过 {@link GTUF_Machines} 的公开工厂注册增强仓（dev-only 验证工厂 API，
 * 让 {@code PARALLEL_HATCH_TEST} / {@code INDUSTRIAL_STEAM_MIXER} 测试机可放入增强仓）。
 * 注意 dev id 前缀 {@code dev_} 与脚本示例 id 不同，避免 dev 环境内冲突；
 * 正式环境这些仓由整合包作者的 KubeJS 脚本注册。</p>
 */
